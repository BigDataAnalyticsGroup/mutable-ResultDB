...
class JoinGraph:
    ...
    def get_relation(self, r_id: int) -> Relation | None:
    ...
    def get_projections(self, relations: set[Relation]) -> list[str]:
        """ Compute the list of projections contained in `relations`.

        Parameters
        ----------
        - relations: set[Relation]
            The relation for which to compute the corresponding projections.

        Returns
        -------
        A list of projections contained in `relations`.

        """
        projections: list[str] = list()
        for relation in relations:
            if relation.projections:
                prefix = relation.alias if relation.alias else relation.name
                projections.extend([f'{prefix}.{p}' for p in relation.projections])
        return projections
    ...
    def neighbors(self, r: Relation) -> set[Relation]:
        ...
        neighbors = set()
        for j in self.joins:
            if (r.id == j.left_relation.id):
                neighbors.add(j.right_relation)
            elif (r.id == j.right_relation.id):
