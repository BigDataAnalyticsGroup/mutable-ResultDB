...
* Start an interactive shell in the `mutable` docker container.
    ...
    ```
* Download IMDb dataset. Executed inside the docker container.
    ```console
    $ pipenv run python ./benchmark/get_data.py job
    ```
* Set up PostgreSQL database. Executed inside the docker container.
    ```console
