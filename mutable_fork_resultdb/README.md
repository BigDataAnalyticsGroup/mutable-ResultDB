...
* Download IMDb dataset.
    ```console
    $ python ./benchmark/job/get_data.py job
    ```
...
* Join Order Benchmark
    ...
    ```console
    $ PYTHONPATH=./benchmark/result-db/utils python ./benchmark/result-db/result-set-sizes/compute_result_set_size.py -u <username>
    ```
...
* Join Order Benchmark
    - To generate the rewrite methods, execute the following script.
        ```console
        $ PYTHONPATH=./benchmark/result-db/utils python ./benchmark/result-db/rewrite-methods/generate_rewrite_methods.py -q imdb -o benchmark/result-db/rewrite-methods/job/ --no-data-transfer
        ```
    ...
    - Run each query using the following command. **Make sure to add your PostgreSQL username!**
        ```console
        $ PYTHONPATH=./benchmark/result-db/utils python ./benchmark/result-db/rewrite-methods/run.py -u <username> -d imdb -n 5 -q q1a --directory ./benchmark/result-db/rewrite-methods/job
        ```
    - The results can be found in `./benchmark/result-db/rewrite-methods/job/rewrite-results.csv`.
* Synthetic Star Schema
    - To generate the rewrite methods, execute the following script.
        ```console
        $ PYTHONPATH=./benchmark/result-db/utils python ./benchmark/result-db/rewrite-methods/generate_rewrite_methods.py -q star -o benchmark/result-db/rewrite-methods/star/ --no-data-transfer
        ```
    - Analogous to the JOB experiments, run each query. **Again, make sure to add your PostgreSQL username!**
        ```console
        $ PYTHONPATH=./benchmark/result-db/utils python ./benchmark/result-db/rewrite-methods/run.py -u <username> -d star -n 5 -q star --directory ./benchmark/result-db/rewrite-methods/star
        ```
...
* Join Order Benchmark
    ...
    ```console
    $ PYTHONPATH=./benchmark/result-db/utils python ./benchmark/result-db/algorithm/create_injected_cardinalities.py -u <username> -d imdb -q imdb -o ./benchmark/result-db/algorithm/job/
    ```
...
* Synthetic Star Schema
    - Generate the real cardinalities for injection into mutable. **Make sure to add your PostgreSQL username!**
    ```console
    $ PYTHONPATH=./benchmark/result-db/utils python ./benchmark/result-db/algorithm/create_injected_cardinalities.py -u <username> -d star -q star -o ./benchmark/result-db/algorithm/star/
    ```
    - Run the benchmark scripts.
    ```console
